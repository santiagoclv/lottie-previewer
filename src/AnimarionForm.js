...
class AnimarionForm extends React.Component {
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        const { placeholders, dragger } = values;

        if (dragger) {
          let reader = new FileReader();
          reader.onload = () => {
            let animationTemplate = reader.result;
  ...
  render() {
    ...
    const file = getFieldValue("dragger");
      ...
      <Form {...formItemLayoutItem} onSubmit={this.handleSubmit}>
        <Form.Item  {...formItemLayoutWithOutLabel} style={{ width: 'calc(100% - 20px)', margin: '10px' }}>
          {getFieldDecorator('dragger', {
            ...
            <Upload.Dragger required={true} multiple={false} name="file" beforeUpload={() => false} fileList={file ? [file] : undefined}>
